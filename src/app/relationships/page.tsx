...
export default function RelationshipsPage() {
  ...
  useEffect(() => {
    ...
    if (!session.user.isApprovedByAdmin) return router.push("/pending-approval");
    fetchRelationships();
  }, [session, status]);
  ...
  const handleAddRelationship = async () => {
    if (!selectedUser || !relationType) return toast.error("Select user & relationship type");
    setSubmitting(true);
    try {
      const res = await fetch("/api/relationships", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ personId2: selectedUser._id, relationType, description })
      });
  ...
  const handleEditRelationship = async () => {
    if (!selectedRelationship || !relationType) return toast.error("Select relationship type");
    setSubmitting(true);
    try {
      const res = await fetch(`/api/relationships/${selectedRelationship._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        setRelationships(rels => rels.map(r => r._id === selectedRelationship._id ? data.relationship : r));
        resetEditDialog();
      } else toast.error(data.error || "Failed");
    } catch {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100">
      ...
      <nav className="bg-white/80 backdrop-blur-md border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center h-16">
          <Link href="/dashboard" className="flex items-center space-x-3">
            ...
            <span className="text-xl font-semibold text-gray-900">AVS Family Tree</span>
          </Link>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10 space-y-10">
        ...
        <div>
          ...
          <p className="text-gray-600 text-sm">Manage your family connections and relationships</p>
        </div>
        ...
        <Card className="border-0 shadow-md" >
          <CardHeader className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>Connect with family members to grow your tree</CardDescription>
            </div>
            <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen} >
              ...
              <DialogContent className="max-w-3xl w-full p-6 space-y-5" style={{maxWidth: '800px'}}>
                <DialogHeader>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex gap-3 text-xs text-gray-600">
                            {selectedUser.gothiram && <span><User className="inline h-3 w-3 mr-1" />{selectedUser.gothiram}</span>}
                            {selectedUser.nativePlace && <span><MapPin className="inline h-3 w-3 mr-1" />{selectedUser.nativePlace}</span>}
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <Card className="border-0 shadow-md">
          <CardHeader>
            ...
            <CardDescription>View and manage all your connections</CardDescription>
          </CardHeader>
          <CardContent>
            {relationships.length === 0 ? (
              <div className="text-center py-16">
                <Users className="h-14 w-14 text-gray-300 mx-auto mb-4" />
                <h3 className="text-lg font-semibold text-gray-900 mb-1">No Relationships Yet</h3>
                <p className="text-gray-600 text-sm mb-5">Start building your family tree by adding your first relationship</p>
                <Button onClick={() => setAddDialogOpen(true)} className="avs-button-primary">
                  <Plus className="h-4 w-4 mr-2" /> Add Relationship
                </Button>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <Badge variant="outline" className="border-[#E63946] text-[#E63946]">{rel.relationType}</Badge>
                            {rel.isApproved ? (
                          ...
                          <div className="flex flex-wrap gap-3 text-xs text-gray-600">
                            {person.gothiram && <span><User className="inline h-3 w-3 mr-1" />{person.gothiram}</span>}
                            {person.nativePlace && <span><MapPin className="inline h-3 w-3 mr-1" />{person.nativePlace}</span>}
                            {rel.description && <span className="italic text-gray-500">{rel.description}</span>}
                          </div>
                          {rel.createdBy && (
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" size="sm" className="text-red-600 hover:bg-red-50" onClick={() => handleDeleteRelationship(rel._id)}>
        ...
        <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
          <DialogContent className="max-w-md w-full p-6 space-y-5">
            ...
            <DialogFooter>
              ...
              <Button onClick={handleEditRelationship} disabled={!relationType || submitting} className="avs-button-primary">
